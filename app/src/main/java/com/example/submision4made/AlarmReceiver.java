...
public class AlarmReceiver extends BroadcastReceiver {
                ...
                public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < list.length(); i++) {
                            ...
                            showAlarmNotification(thisContext, type, null, ID_RELEASE_TODAY_REMINDER, movieItem);
                        }
    ...
    private void showAlarmNotification(Context context, String title, String message, int notifId, Movies movieItem) {
        ...
        if (listItems.size() != 0) {
            for (int i = 0; i < listItems.size(); i++) {
                inboxStyle.addLine(listItems.get(i).getTitle());
            }
        }
        ...
        PendingIntent pendingIntent = getPendingIntent(context, notifId, movieItem);
        ...
        if (notificationManager != null) {
            notificationManager.notify(ID_RELEASE_TODAY_REMINDER, notification);
        }
    }

    private PendingIntent getPendingIntent(Context context, int notificationId, Movies item) {
        Intent intent;
        if (notificationId == ID_RELEASE_TODAY_REMINDER) {
            intent = new Intent(context, DetailActivity.class);
            intent.putExtra(DetailActivity.MOVIE_INDEX, item);
        } else {
            return null;
        }
